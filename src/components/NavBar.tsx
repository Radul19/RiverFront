...
const NavBar = ({ active }: { active: number }) => {
  ...
  return (
    <View style={st.navbar_ctn}>
      <Pressable style={({ pressed }) => ({ opacity: pressed ? 0.5 : 1 })} onPress={goHome}>
        {active === 1 ? <IconHomeFill /> : <IconHomeLine />}
      </Pressable>
      <Pressable style={({ pressed }) => ({ opacity: pressed ? 0.5 : 1 })} onPress={goFav}>
        {active === 2 ? <IconHeart /> : <IconHeartLine />}
      </Pressable>
      <Pressable style={({ pressed }) => ({ opacity: pressed ? 0.5 : 1 })} onPress={goMarket}>
        {active === 3 ? <IconStall /> : <IconStallLine />}
      </Pressable>
      <Pressable style={({ pressed }) => ({ opacity: pressed ? 0.5 : 1 })} onPress={goProfile}>
